        ...
        if (!TextUtils.isEmpty(response))
        {
            try
            {
                ...
                for (int i = 0; i < allCountrys.length(); i++) {
                    JSONObject countryOj = allCountrys.getJSONObject(i);
                    System.out.println(countryOj.getString("name")+"   "+countryOj.getString("weather_id"));
                    Country country = new Country(countryOj.getString("name"),countryOj.getString("weather_id"),cityId,false);
                    country.save();
                }
                return true;
            }
